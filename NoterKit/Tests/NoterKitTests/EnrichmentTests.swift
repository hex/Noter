// ABOUTME: Tests for enrichment shared by every app: the prompt, the API requests, how each reply
// ABOUTME: envelope is read, and how the result is applied to a note. No network, no processes.
...
struct EnrichmentTests {
    ...
    func anthropicRequest() throws {
        let r = try EnrichmentAPI.request(prompt: "P", model: "claude-haiku-4-5", provider: .anthropic, apiKey: "k")
    ...
    func openAIRequest() throws {
        let r = try EnrichmentAPI.request(prompt: "P", model: "gpt-5.4-mini", provider: .openai, apiKey: "k")
    ...
    func geminiRequest() throws {
        let r = try EnrichmentAPI.request(prompt: "P", model: "gemini-3.8-flash", provider: .gemini, apiKey: "k")
    ...
    func parseAPIs() throws {
        ...
        for reply in [anthropic, openai, gemini] {
            #expect(try EnrichmentAPI.parse(Data(reply.utf8)).title == "T")
        }
    }
    ...
    func parseAPIError() {
        ...
        #expect(throws: EnrichmentError.remote("invalid x-api-key")) { try EnrichmentAPI.parse(Data(reply.utf8)) }
    }
    ...
    func parseCodex() throws {
        ...
        #expect(try EnrichmentAPI.parse(Data(out.utf8)).title == "T")
    }
    ...
    func parseAgy() throws {
        ...
        let e = try EnrichmentAPI.parse(Data(out.utf8))
        #expect(e.title == "T")
    ...
    func parseStructured() throws {
        ...
        #expect(try EnrichmentAPI.parse(Data(out.utf8)).title == "Right")
    }
    ...
    func parseTwoObjects() throws {
        ...
        #expect(try EnrichmentAPI.parse(Data(out.utf8)).title == "A")
    }
    ...
    func parseClaude() throws {
        ...
        #expect(try EnrichmentAPI.parse(Data(out.utf8)).color == "rose")
    }

    @Test("A CLI error envelope is reported")
    func parseError() {
        let out = #"{"status":"ERROR","response":"","error":"invalid model selection"}"#
        #expect(throws: EnrichmentError.remote("invalid model selection")) { try EnrichmentAPI.parse(Data(out.utf8)) }
    }

    @Test("A reply without JSON is a bad reply")
    func parseNoJSON() {
        #expect(throws: EnrichmentError.badReply("no JSON in reply")) { try EnrichmentAPI.parse(Data("nothing here".utf8)) }
    }

    @Test("A link preview's text reaches the prompt")
    func previewInPrompt() {
        var n = note
        n.preview = LinkPreview(url: URL(string: "https://x.com/a/status/1")!, title: "Author", description: "the tweet body", siteName: "X")
        #expect(EnrichmentAPI.prompt(for: n).contains("the tweet body"))
    }

    @Test("The prompt carries the captured content and any attachment paths")
    func prompt() {
        #expect(EnrichmentAPI.prompt(for: note).contains("https://x.com/someone/status/1"))
        let url = URL(fileURLWithPath: "/tmp/att/receipt.jpg")
        #expect(EnrichmentAPI.prompt(for: note, attachments: [url]).contains("/tmp/att/receipt.jpg"))
    }
    ...
    func apply() {
        ...
        let out = EnrichmentAPI.apply(e, to: note)
        #expect(out.title == "T")
    ...
    func applyWithPreview() {
        var n = note
        n.preview = LinkPreview(url: URL(string: "https://x.com/someone/status/1")!, title: "A", siteName: "X")
        let out = EnrichmentAPI.apply(Enrichment(title: "T", summary: "S", tags: [], color: "mint"), to: n)
    ...
    func badColor() {
        ...
        #expect(EnrichmentAPI.apply(e, to: note).colorName == "sky")
    }
}
